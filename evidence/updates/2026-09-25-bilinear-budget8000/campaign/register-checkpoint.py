...
root = Path.cwd()
run = root / "runs/learning/bilinear-budget8000-v1"
prior_path = (
    root / "docs/experiments/2026-09-25-bilinear-worst-boundary-model-version-inventory.json"
)
out = root / "docs/experiments/2026-09-25-bilinear-budget8000-model-version-inventory.json"
require(not out.exists(), "immutable inventory")
require(
    sha(prior_path) == "d18c7cd521d758593fbad803452dde6fa820e542fed52b7ae4557025e2ebfa2a",
...
require(prior["registered_final_count"] == 34, "prior inventory count")
require(
...
relative = "runs/learning/bilinear-budget8000-v1/checkpoint-final.pt"
expected = dict(prior["verified_final_checkpoint_bytes"])
require(len(expected) == 34 and relative not in expected, "new distinct derivative")
...
require(len(expected) == 35, "final count")
...
require(
    sidecar["training_metadata"]["objective"] == "plm-bilinear-residual-balanced-bce-v1"
    and sidecar["experiment_identity"]["evaluator_version"] == "plm-bilinear-budget8000-screen-v1"
    ...
    "unchanged objective and original-parent lineage",
)
require(
    sidecar["global_step"] == 8000
    and sidecar["training_metadata"]["parent_training_steps"] == 2000
    and sidecar["training_metadata"]["residual_updates"] == 8000,
    "separate parent and residual steps",
)
result = {
    "schema_version": 1,
    "inventory_kind": "additive-bilinear-budget8000-derivative-and-final-byte-verification-v1",
    "observed_at_utc": datetime.now(UTC).isoformat(),
    "prior_inventory_path": prior_path.relative_to(root).as_posix(),
    "prior_inventory_sha256": sha(prior_path),
    "prior_registered_count": 34,
    "added_count": 1,
    "registered_final_count": 35,
    "verified_final_checkpoint_bytes": expected,
    "new_artifact": {
        "run_id": "bilinear-budget8000-v1",
    ...
    "limitations": [
        "Fresh byte hashes cover all35 final files; older payloads/configs not revalidated.",
...
print(json.dumps({"verified_final_count": 35, "inventory_sha256": sha(out)}))
