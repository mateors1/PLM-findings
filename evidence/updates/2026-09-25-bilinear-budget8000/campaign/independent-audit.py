"""Independent 8000-update bilinear audit; CPU saved-payload inspection.
...
PLAN_SHA = "064eaa115e673caaf89196bd391005a42441b6fb54cd49f9d67a6ab9c3aaab6f"
...
RUNNER_SHA = "cd593c14c429247837c26b09754509d5cfd1dd36adb317f56522a80a78a25122"
RECIPE_SHA = "f976442315433e32d5c88ce2f4e911509ec5ea56542fc2a247893cefd0ddfefc"
RUNNER_RECEIPT_SHA = "8e0f785207ec1e64d5eb87d426db3c19645a5db2b4aae98fe4ebbb730d0a6387"
...
def prefix_diagnostic(current, historical, historical_final):
    require(len(current) == 8000 and len(historical) == 2000, "fixed prefix coverage")
    finite(historical_final)
    require(historical_final >= 0, "historical endpoint nonnegative")
    mismatches = []
    for index, (old, new) in enumerate(zip(historical, current[:2000], strict=True)):
        require(old["update"] == new["update"] == index + 1, "aligned prefix updates")
        finite(old["pre_update_loss"])
        finite(new["pre_update_loss"])
        if old["pre_update_loss"] != new["pre_update_loss"]:
            mismatches.append(index)
    require(current[2000]["update"] == 2001, "post2000 corresponds to pre2001")
    endpoint = current[2000]["pre_update_loss"]
    finite(endpoint)
    return {
        "pre_update_compared_count": 2000,
        "all_pre_update_equal": not mismatches,
        "mismatch_count": len(mismatches),
        "first_mismatch_update": mismatches[0] + 1 if mismatches else None,
        "historical_final_post_update_loss": historical_final,
        "current_pre_update_2001_loss": endpoint,
        "boundary_loss_equal": historical_final == endpoint,
...
def recipe_contract(value, inherited):
    ...
    exact(value, expected, "sole declared budget, evaluator and gate changes")
...
def provenance(summary, folder, inherited):
    require(
        summary["campaign_version"] == "bilinear-budget8000-v1"
...
def audit(summary_path):
    ...
    prefix = prefix_diagnostic(
        reports["training.json"]["history"],
        historical_training["history"],
        historical_training["final_post_update_loss"],
    )
    exact(
        reports["training.json"]["training_prefix_diagnostic"],
        prefix,
        "independent descriptive prefix comparison",
    )
    exact(summary["training_prefix_diagnostic"], prefix, "summary prefix comparison")
    exact(
    ...
    tests = ROOT / "runs/learning/bilinear-budget8000-auditor-tests-v1"
