...
destination = root / "docs/experiments/2026-09-25-bilinear-budget8000.json"
...
require(training["complete"] is True and training["completed_updates"] == 8000, "complete fit")
...
require(len(training["history"]) == 8000, "complete 8000-row trace")
require([r["update"] for r in training["history"]] == list(range(1, 8001)), "trace order")
parts = []
reconstructed = []
for part in range(1, 5):
    start, end = (part - 1) * 2000, part * 2000
    history_path = destination.with_name(destination.stem + f"-training-part-{part:02}.json")
    require(not history_path.exists(), "immutable training chunk")
    rows = training["history"][start:end]
    write(
        history_path,
        {
            "source_training_sha256": sha(run / "training.json"),
...
require(
    ...
    "prefix diagnostic receipt agreement",
)
portable = {
    "date": "2026-09-25",
    "campaign_version": summary["campaign_version"],
