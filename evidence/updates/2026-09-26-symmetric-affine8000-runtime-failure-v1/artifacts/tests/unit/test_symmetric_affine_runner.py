...
def runner():
    return load("refit_symmetric_affine")
...
def recipe_pair():
    return [
        ...
        for name in ("symmetric_affine8000_v1.json", "bilinear_budget8000_v1.json")
    ]
...
def test_import_has_no_eager_torch():
    ...
    tree = ast.parse((ROOT / "scripts/refit_symmetric_affine.py").read_text(encoding="utf-8"))
...
def test_runner_has_no_whole_corpus_loader_or_inherited_preflight():
    ...
    source = (ROOT / "scripts/refit_symmetric_affine.py").read_text(encoding="utf-8")
...
def test_preflight_opens_only_selected_allowlist(runner, mean, tmp_path, monkeypatch):
    ...
    objects = {
        ...
        tmp_path / "configs/experiments/symmetric_affine8000_v1.json": new_recipe,
    }
    calls = []
    ...
    result = runner._preflight(
        ...
        tmp_path / "configs/experiments/symmetric_affine8000_v1.json",
        bilinear,
...
@pytest.mark.parametrize(
    "mutation", [None, "vocabulary", "objective", "step", "config", "evaluator", "corpus"]
)
def test_selected_runtime_uses_only_checkpoint_and_vocabulary(
    ...
    model_config.model_copy = lambda update: model_config
    model_config.model_dump = lambda mode: {"dim": 256}
    config = SimpleNamespace(
        model=model_config, train=SimpleNamespace(seq_len=512), eval=SimpleNamespace(device="cuda")
    )
    ...
    metadata = {"objective": "causal-next-token-v1", "model_config": {}}
    context = {
        "parent_run": {"config": {"synthetic": True}, "identity": identity},
        "parent_sidecar": {"training_metadata": metadata},
        "vocabulary_path": tmp_path / "vocabulary.json",
        "inherited_architecture": {"dim": 256},
        "parent": tmp_path / "checkpoint.pt",
        "corpus_identity": corpus,
        "partitions": {"train": ("train",), "validation": ("validation",)},
    }
    raw = {
        "config": context["parent_run"]["config"],
        "corpus_identity": corpus,
        "training_metadata": metadata,
    }
    ...
    modules = {
        "plm.config": {
            "RootConfig": SimpleNamespace(model_validate=lambda raw: config),
            "ModelConfig": SimpleNamespace(model_validate=lambda raw: model_config),
        },
...
def test_production_readiness_paths_reject_unknown_before_open(tmp_path, entry):
    ...
    module = load("refit_symmetric_affine")
    opened = []
