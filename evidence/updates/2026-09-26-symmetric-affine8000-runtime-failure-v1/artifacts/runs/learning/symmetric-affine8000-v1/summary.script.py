...
_PRIMARY_RECEIPT_PATHS = (
    "runs/learning/symmetric-affine-runner-tests-v1/receipt.json",
    "runs/learning/symmetric-affine-runner-tests-v1/test-receipt.json",
)
_AUDITOR_CONTRACT_PATH = "runs/learning/symmetric-affine-auditor-tests-v1/readiness.json"
_AUDITOR_PART_PATHS = {
    "auditor": "runs/learning/symmetric-affine8000-v1/independent-audit.py",
    "tests": "tests/unit/test_symmetric_affine_audit.py",
    "test_receipt": "runs/learning/symmetric-affine-auditor-tests-v1/test-receipt.json",
}
...
_EVALUATOR = "plm-symmetric-affine8000-screen-v1"
...
def _preflight(
    ...
    _require(
        ...
        == (root / "configs/experiments/symmetric_affine8000_v1.json").resolve(),
...
def _load_selected_runtime(context):
    ...
    _require(
        ...
        and state.checkpoint_hash
        == "e844dd73c3af4ae794033ed356f358bf0541dd2469002554242993e596dcd4e1"
        and raw["config"] == context["parent_run"]["config"]
        and raw["corpus_identity"] == context["corpus_identity"]
        and raw["training_metadata"] == context["parent_sidecar"]["training_metadata"]
        and raw["training_metadata"]["objective"] == "causal-next-token-v1"
        and ModelConfig.model_validate(raw["training_metadata"]["model_config"]) == model_config
        and identity["evaluator_version"] == "plm-train-causal-v2"
        and all(identity[k] == v for k, v in context["corpus_identity"].items()),
...
def _auditor_contract(root, path, pinned, helper, inputs):
    ...
    _require(
        ...
        and contract.get("experiment") == "symmetric-affine8000-v1"
...
def main():
    ...
    parser.add_argument(
        "--recipe", type=Path, default=Path("configs/experiments/symmetric_affine8000_v1.json")
    )
    parser.add_argument(
        "--out", type=Path, default=Path("runs/learning/symmetric-affine8000-v1/summary.json")
    )
    ...
    required = [
        ...
        root / "tests/unit/test_symmetric_affine_runner.py",
