...
def seed_context(seed, historical):
    ...
    exact(run["training_metadata"], sidecar["training_metadata"], "parent metadata sidecar")
...
def provenance(summary, folder, historical, context, *, aggregate=False):
    ...
    initial_inputs = read(folder / "summary.inputs.json")
    if aggregate:
        aggregate_inventory_contract(summary["preflight_input_sha256"], initial_inputs, inputs)
        path = Path(summary["aggregate_inputs_path"])
        require(
            path == folder / "summary.aggregate-inputs.json", "aggregate dynamic inventory path"
        )
    ...
    if not aggregate:
        ...
        source = archive_inventory(folder / "summary.source.zip", BUDGET.SOURCE_SHA, runtime)
        configs = archive_inventory(folder / "summary.configs.zip", BUDGET.CONFIGS_SHA, runtime)
...
def execution_receipt(folder, seed, summary_sha):
    ...
    start, finish = terminal_contract(receipt, seed, summary_sha, sha(stdout), previous)
    if seed == 1731:
    ...
    if launch.exists():
        value = read(launch)
        require(
            value["seed"] == seed and value["started_at_utc"] == receipt["started_at_utc"],
            "launch versus terminal chronology",
        )
    return {
        "path": str(path),
        "sha256": sha(path),
...
def own_receipt():
    folder = ROOT / "runs/learning/bilinear-seed-replication-auditor-tests-v1"
...
def audit_aggregate(summary_path):
    require(summary_path.parent == HERE, "aggregate campaign directory")
    result = audit_header(summary_path)
    summary = read(summary_path)
    historical = historical_context()
