...
prior_path = root / "docs/experiments/2026-09-25-bilinear-budget2000-model-version-inventory.json"
out = root / "docs/experiments/2026-09-25-bilinear-seed-replication-model-version-inventory.json"
require(not out.exists(), "immutable inventory")
require(
    sha(prior_path) == "6fdfd6aa9780152f4c268c650b82e0f7453313db606dc068fd0718489c740748",
...
require(sha(run / "summary.json") == decision["summary_sha256"], "audited aggregate identity")
require(sha(run / "independent-audit.json") == decision["audit_sha256"], "audited aggregate")
require(prior["registered_final_count"] == 31, "prior inventory count")
expected = dict(prior["verified_final_checkpoint_bytes"])
require(len(expected) == 31, "prior final files")
parents = {
...
for child in decision["children"]:
    ...
    require(sha(folder / "independent-audit.json") == child["audit_sha256"], "audited seed")
    ...
    require(
        ...
        == "plm-bilinear-seed-replication-v1",
...
require(len(expected) == 33 and found == set(expected), "final inventory scope")
for path, digest in expected.items():
    require(sha(root / path) == digest, "checkpoint bytes changed: " + path)
result = {
    "schema_version": 1,
    "inventory_kind": "additive-bilinear-two-seed-derivatives-and-final-byte-verification-v1",
    "observed_at_utc": datetime.now(UTC).isoformat(),
    "prior_inventory_path": prior_path.relative_to(root).as_posix(),
    "prior_inventory_sha256": sha(prior_path),
    "prior_registered_count": 31,
    "added_count": 2,
    "registered_final_count": 33,
    "verified_final_checkpoint_bytes": expected,
    "new_artifacts": added,
    "decision_sha256": args.decision_sha256,
    "script_sha256": sha(__file__),
    "limitations": [
        "Fresh byte hashes cover33final files; older payloads/configs not all revalidated.",
...
print(json.dumps({"verified_final_count": 33, "inventory_sha256": sha(out)}))
