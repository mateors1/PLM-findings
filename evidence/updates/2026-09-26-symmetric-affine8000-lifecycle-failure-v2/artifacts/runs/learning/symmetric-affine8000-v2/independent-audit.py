...
REPAIR_SHA = "8ac4fbc66d40bf26e249d0ee9a0cd9117d06653e762552629c98e2b53a2c5a18"
...
EVALUATOR = "plm-symmetric-affine8000-screen-v2"
...
AUDITOR_TEST_DEPENDENCIES = (
    "runs/learning/symmetric-affine8000-v2/independent-audit.py",
    "tests/unit/test_symmetric_affine_audit_v2.py",
...
def dependency_allowlist():
    fixed = {
        ...
        "docs/experiments/2026-09-26-symmetric-affine-input-amendment.md",
        "docs/experiments/2026-09-26-symmetric-affine-runtime-repair-v2.md",
        "configs/experiments/symmetric_affine8000_v2.json",
        "scripts/refit_symmetric_affine_v2.py",
    ...
    for name in (
        ...
        fixed.add("runs/learning/symmetric-affine8000-v2/" + name)
    for suffix in (
        "script.py",
        "affine-helper.py",
        "bilinear-helper.py",
        ...
        fixed.add("runs/learning/symmetric-affine8000-v2/summary." + suffix)
    for directory in ("symmetric-affine-auditor-tests-v2", "symmetric-affine-runner-tests-v2"):
        for name in (
            "test-receipt.json",
            "receipt.json",
            "stdout.txt",
...
def recipe_contract(value, inherited):
    ...
    expected.update(
        ...
        repair_declaration_sha256=REPAIR_SHA,
...
def receipt_contract(receipt):
    require(
        ...
        and receipt.get("repair_declaration_sha256") == REPAIR_SHA,
...
def readiness_contract(reference, inputs):
    ...
    require(
        ...
        and contract["experiment"] == "symmetric-affine8000-v2"
        ...
        and contract["repair_declaration_sha256"] == REPAIR_SHA,
        "readiness identity",
    )
    bound = {}
...
def provenance(summary, folder, inherited):
    require(
        summary["campaign_version"] == "symmetric-affine8000-v2"
    ...
    require(
        dependencies[str(ROOT / "scripts/refit_symmetric_affine_v2.py")]
        == summary["script_sha256"],
        "tested exact runner",
    )
...
def parent_admission(parent, sidecar, parent_run, inherited, vocabulary_size):
    """All parent predicates, using the checkpoint's actual TrainConfig schema."""
    root_config = parent_run["config"]
    resolved_model = dict(root_config["model"])
    resolved_model.update(vocab_size=vocabulary_size, max_seq_len=root_config["train"]["seq_len"])
    metadata = parent["training_metadata"]
    identity = parent["experiment_identity"]
    checks = {
        "global_step_2000": type(parent["global_step"]) is int and parent["global_step"] == 2000,
        "original_checkpoint_hash": sidecar["checkpoint_hash"] == PARENT_SHA,
        "checkpoint_config_equals_run_train_config": parent["config"] == root_config["train"],
        "corpus_identity_equals_inherited": parent["corpus_identity"]
        == inherited["corpus_identity"],
        "training_metadata_equals_sidecar": metadata == sidecar["training_metadata"],
        "training_objective_causal_next_token": metadata["objective"] == "causal-next-token-v1",
        "training_model_config_equals_resolved_model": metadata["model_config"] == resolved_model,
        "training_evaluator_current": identity["evaluator_version"] == "plm-train-causal-v2",
        "training_identity_corpus_fields_match": all(
            identity.get(k) == v for k, v in parent["corpus_identity"].items()
        ),
    }
    require(
        all(checks.values()),
        "parent admission predicates: " + str([k for k, v in checks.items() if not v]),
    )
    exact(identity, parent_run["identity"], "original parent run identity")
    exact(resolved_model, inherited["inherited_architecture"], "accepted resolved model identity")
    require(parent["split_hash"] == sidecar["split_hash"] == SPLIT_SHA, "original parent split")
    return checks
...
def training_audit(summary, reports, recipe, context, folder):
    ...
    parent_checks = parent_admission(parent, parent_sidecar, old_run, _old, len(vocabulary))
    ...
    return {
        ...
        "parent_admission_checks": parent_checks,
