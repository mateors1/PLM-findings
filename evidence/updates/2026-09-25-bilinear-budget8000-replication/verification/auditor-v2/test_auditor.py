...
PATH = (
    ...
    / "bilinear-budget8000-replication-v1/independent-audit-v2.py"
)
...
def test_output_overwrite_refused(tmp_path, monkeypatch):
    path = tmp_path / "independent-audit-v2.json"
...
PRIMARY_RECEIPTS = (
    Path(__file__).resolve().parents[1] / "bilinear-budget8000-replication-runner-tests-v1"
)


def prerequisite_receipt(name, digest):
    path = PRIMARY_RECEIPTS / name
    assert hashlib.sha256(path.read_bytes()).hexdigest() == digest
    return json.loads(path.read_text())


def test_real_seed17_receipt_production_inventory_path():
    receipt = prerequisite_receipt("receipt.json", A.RUNNER_RECEIPT_SHA)
    assert len(receipt["test_dependencies"]) == 17
    A.seed_dependency_inventory(receipt)


def test_original_real_aggregate4_receipt_production_inventory_path():
    receipt = prerequisite_receipt("aggregate-receipt.json", A.V1_AGGREGATION_RECEIPT_SHA)
    old_roles = [
    ...
    assert len(receipt["test_dependencies"]) == 4
    A.dependency_inventory_contract(
        receipt, [A.ROOT / p for p in old_roles], "original aggregate inventory"
    )


def test_new_aggregate6_production_inventory_path():
    receipt = {
        "test_dependencies": {str(A.ROOT / p): "a" * 64 for p in A.AGGREGATE_TEST_DEPENDENCIES}
    }
    assert len(receipt["test_dependencies"]) == 6
    A.aggregate_dependency_inventory(receipt)


@pytest.mark.parametrize("kind", ["seed", "aggregate"])
@pytest.mark.parametrize("mutation", ["missing", "extra", "list", "set", "nonstring_key"])
def test_exact_production_inventory_rejects_invalid_schemas(kind, mutation):
    paths = A.SEED_TEST_DEPENDENCIES if kind == "seed" else A.AGGREGATE_TEST_DEPENDENCIES
    check = A.seed_dependency_inventory if kind == "seed" else A.aggregate_dependency_inventory
    receipt = {"test_dependencies": {str(A.ROOT / p): "a" * 64 for p in paths}}
    if mutation == "missing":
        receipt["test_dependencies"].pop(next(iter(receipt["test_dependencies"])))
    elif mutation == "extra":
        receipt["test_dependencies"]["unexpected.py"] = "a" * 64
    elif mutation == "list":
        receipt["test_dependencies"] = list(receipt["test_dependencies"])
    elif mutation == "set":
        receipt["test_dependencies"] = set(receipt["test_dependencies"])
    else:
        receipt["test_dependencies"][1] = "a" * 64
    with pytest.raises(ValueError):
        check(receipt)


def test_original_json_helper_failure_is_regressed_without_weakening_it():
    with pytest.raises(TypeError, match="not JSON serializable"):
        A.exact({"a"}, {"a"}, "demonstrated v1 boundary")
    A.dependency_inventory_contract(
        {"test_dependencies": {"a": "hash"}}, ["a"], "safe set equality"
    )


def parsed_functions(path):
    return {
        node.name: node
...
def test_production_call_sites_use_exercised_inventory_helpers():
    functions = parsed_functions(PATH)
    for caller, callee in [
        ("provenance", "seed_dependency_inventory"),
        ("aggregate_provenance", "aggregate_dependency_inventory"),
    ]:
        calls = [node for node in ast.walk(functions[caller]) if isinstance(node, ast.Call)]
        assert any(isinstance(node.func, ast.Name) and node.func.id == callee for node in calls)
    for node in ast.walk(ast.parse(PATH.read_text())):
        if (
            isinstance(node, ast.Call)
            and isinstance(node.func, ast.Name)
            and node.func.id == "exact"
        ):
            assert not any(
                isinstance(arg, (ast.Set, ast.SetComp))
                or (
...
def test_repair_preserves_scientific_contract_functions():
    original = PATH.with_name("independent-audit.py")
    assert hashlib.sha256(original.read_bytes()).hexdigest() == A.V1_AUDITOR_SHA
    before, after = parsed_functions(original), parsed_functions(PATH)
    for name in [
        "recipe_contract",
        "lineage_contract",
        "loss_contract",
...
def test_real_v2_aggregate6_receipt_production_inventory_path():
    receipt = prerequisite_receipt("aggregate-receipt-v2.json", A.AGGREGATION_RECEIPT_SHA)
    assert len(receipt["test_dependencies"]) == 6
    A.aggregate_dependency_inventory(receipt)
