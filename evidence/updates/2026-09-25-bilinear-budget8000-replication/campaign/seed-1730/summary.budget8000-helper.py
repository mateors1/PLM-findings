"""Fixed 8000-update balanced-BCE sibling with descriptive scalar-prefix comparison.

Own evaluator-specific lifecycle; frozen scorer/fit with no intermediate validation.
"""
...
_PLAN = "064eaa115e673caaf89196bd391005a42441b6fb54cd49f9d67a6ab9c3aaab6f"
...
_EVALUATOR = "plm-bilinear-budget8000-screen-v1"
...
def _recipe(value, inherited):
    ...
    expected.update(
        experiment="bilinear-budget8000-v1",
    ...
    _require(
        ...
        "fixed 8000-update recipe differs",
    )
    return value


def _prefix_diagnostic(new_training, historical):
    current, old = new_training["history"], historical["history"]
    _require(len(current) == 8000 and len(old) == 2000, "complete prefix trace coverage")
    _require(
        [r["update"] for r in current] == list(range(1, 8001))
        and [r["update"] for r in old] == list(range(1, 2001)),
        "ordered trace updates",
    )
    differences = [
        i for i in range(2000) if current[i]["pre_update_loss"] != old[i]["pre_update_loss"]
    ]
    first = differences[0] if differences else None
    old_final = historical["final_post_update_loss"]
    new_aligned = current[2000]["pre_update_loss"]
    return {
        "pre_update_compared_count": 2000,
        "all_pre_update_equal": not differences,
        "mismatch_count": len(differences),
        "first_mismatch_update": None if first is None else first + 1,
        "historical_final_post_update_loss": old_final,
        "current_pre_update_2001_loss": new_aligned,
        "boundary_loss_equal": old_final == new_aligned,
...
def _preflight(
    ...
    context.update(
        ...
        historical2000_training=helper._read(old / "training.json"),
...
def _execute(root, out, context, bilinear, mean, helper, summary):
    ...
    try:
        ...
        prefix = _prefix_diagnostic(training, context["historical2000_training"])
        training["training_prefix_diagnostic"] = prefix
        summary["training_prefix_diagnostic"] = prefix
...
def main():
    ...
    parser.add_argument(
        "--plan", type=Path, default=Path("docs/experiments/2026-09-25-bilinear-budget8000-plan.md")
    )
    parser.add_argument(
        "--recipe", type=Path, default=Path("configs/experiments/bilinear_budget8000_v1.json")
    )
    parser.add_argument(
        "--out", type=Path, default=Path("runs/learning/bilinear-budget8000-v1/summary.json")
    )
    ...
    summary = {
        "campaign_version": "bilinear-budget8000-v1",
        "complete": False,
        "acceptance": False,
        "architecture": bilinear._architecture(),
        "objective": _OBJECTIVE,
        ...
        "limitations": [
            ...
            (
                "Only declared update budget changes versus historical2000; "
            ...
            "Exact scalar prefix equality is descriptive and cannot prove "
            "identical intermediate weights.",
            "Prefix mismatch does not change the quality gate or authorize retries.",
