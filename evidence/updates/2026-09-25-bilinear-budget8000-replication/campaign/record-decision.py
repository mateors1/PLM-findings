...
require(sha(run / "aggregate.json") == args.summary_sha256, "aggregate primary identity")
require(sha(run / "independent-audit.json") == args.audit_sha256, "aggregate audit identity")
summary, audit = read(run / "aggregate.json"), read(run / "independent-audit.json")
...
aggregate_execution = read(run / "aggregate-execution-receipt.json")
require(
    ...
    and aggregate_execution["stdout_sha256"] == sha(run / "aggregate-stdout.txt"),
...
decision = {
    ...
    "aggregate_execution_receipt_sha256": sha(run / "aggregate-execution-receipt.json"),
    "children": children,
    "gate": audit["gate"],
