...
V1_AGGREGATION_RUNNER_SHA = "78cae68bdf741ec35c6deff6a62b57673092be0cf61968e88bbf40274878789f"
AGGREGATION_RUNNER_SHA = "734e7143f3e4529961bb0cc31bf155c4ac4c60dcb9fecebf72519203bba4b734"
V1_AGGREGATION_RECEIPT_SHA = "0dc5c87d41db7ae3e1a3d9cd591717d0941501df201e836abcad765ead2e3f36"
AGGREGATION_RECEIPT_SHA = "da435a0f299811da17dadc2cc9a780652053e36ea4620bee6cfe8229a56791e8"
REPAIR_PLAN_SHA = "130fd639ecc9b1d771f44f87e1997d1690eb6255b39f85c81f959616f84a89eb"
V1_AUDITOR_SHA = "495db4e3d65e3893fdc68579cad62792c3ab73fd614738b7e97263935af00daa"
V1_TEST_RECEIPT_SHA = "e130b1dee6d8cedae18af6abdf3cdaabeb5ac78668c8e72802357bde6b8e1ad1"
FAILED_AUDIT_RECEIPT_SHA = "d25163d1315aafbc98d9a2cc2bd7f1c7e9c8bdd1355ba27a5d4e3fab2b2cbbb6"
FAILED_AUDIT_STDOUT_SHA = "8c5773b049fc57518f2c389bbd616046d70743704c90a8eba7f9c343732cd8fb"
HISTORICAL_SEEDS = {
    1730: {
...
def provenance(summary, folder, historical, context, *, aggregate=False):
    ...
    if aggregate:
        snapshots.update(REPAIR_BINDINGS)
        snapshots.update(
            {
    ...
    seed_dependency_inventory(receipt)
...
def dependency_inventory_contract(receipt, expected_paths, label):
    """Compare path inventories directly; the JSON equality helper rejects sets."""
    actual = receipt["test_dependencies"]
    require(isinstance(actual, dict), label + " must be a mapping")
    require(all(isinstance(p, str) for p in actual), label + " path types")
    expected = {str(p) for p in expected_paths}
    require(set(actual) == expected, label)


def seed_dependency_inventory(receipt):
    dependency_inventory_contract(
        receipt, (ROOT / p for p in SEED_TEST_DEPENDENCIES), "seed test dependency inventory"
    )


def aggregate_dependency_inventory(receipt):
    dependency_inventory_contract(
        receipt,
        (ROOT / p for p in AGGREGATE_TEST_DEPENDENCIES),
        "aggregation test dependency inventory",
    )


def aggregate_provenance(summary, folder, preflight):
    require(summary["aggregation_version"] == 2, "versioned aggregation")
    evidence = {}
    for suffix, (relative, digest) in REPAIR_BINDINGS.items():
        path = bind(ROOT / relative, digest)
        bind(folder / ("aggregate-v2." + suffix), digest)
        require(preflight.get(str(path)) == digest, "repair evidence consumed before aggregation")
        evidence[suffix] = {"path": str(path), "sha256": digest}
    exact(summary["evidence_repair"], evidence, "versioned aggregation repair inputs")
    require(
    ...
    bind(folder / "aggregate-v2.aggregation-script.py", AGGREGATION_RUNNER_SHA)
    receipt = read(folder / "aggregate-v2.aggregation-test-receipt.json", AGGREGATION_RECEIPT_SHA)
    require(
        receipt["passed"] is True
        and receipt["gpu_used"] is False
        and receipt["experiment_training_executed"] is False
        ...
        and receipt["tested_recipe_sha256"] == RECIPE_SHA
        and receipt["skipped"] == 0
        and receipt["test_count"] == 46,
        "frozen aggregate test receipt",
    )
    aggregate_dependency_inventory(receipt)
    for path, digest in receipt["test_dependencies"].items():
        bind(path, digest)
    ...
    bind(folder / "aggregate-v2.aggregation-test-stdout.txt", receipt["stdout"]["sha256"])
    bind(folder / "aggregate-v2.auditor-script.py", sha(__file__))
    bind(folder / "aggregate-v2.auditor-test-receipt.json", own_receipt())
    require(
...
def own_receipt():
    folder = ROOT / "runs/learning/bilinear-budget8000-replication-auditor-tests-v2"
...
def repair_provenance():
    for relative, digest in REPAIR_BINDINGS.values():
        bind(ROOT / relative, digest)
    bind(HERE / "independent-audit.py", V1_AUDITOR_SHA)
    bind(
        ROOT / "runs/learning/bilinear-budget8000-replication-auditor-tests-v1/test-receipt.json",
        V1_TEST_RECEIPT_SHA,
    )
    bind(
    ...
    folder = HERE / "seed-1730"
    failed = read(folder / "audit-execution-receipt.json", FAILED_AUDIT_RECEIPT_SHA)
    bind(folder / "audit-stdout.txt", FAILED_AUDIT_STDOUT_SHA)
    require(
        failed["exit_code"] == 1
        and failed["audit_sha256"] is None
        and failed["auditor_sha256"] == V1_AUDITOR_SHA
        and failed["test_receipt_sha256"] == V1_TEST_RECEIPT_SHA
        and failed["stdout_sha256"] == FAILED_AUDIT_STDOUT_SHA,
        "retained first audit failure",
    )
    return {
        "declaration_sha256": REPAIR_PLAN_SHA,
        "previous_auditor_sha256": V1_AUDITOR_SHA,
        "previous_test_receipt_sha256": V1_TEST_RECEIPT_SHA,
        "failed_attempt_receipt_sha256": FAILED_AUDIT_RECEIPT_SHA,
        "failed_attempt_stdout_sha256": FAILED_AUDIT_STDOUT_SHA,
        "reason": (
            "Dependency inventories require direct set equality "
            "rather than JSON serialization of sets."
        ),
        "prediction_and_gate_arithmetic_changed": False,
        "primary_seed_artifacts_changed": False,
    }
...
def audit_aggregate(summary_path):
    require(summary_path == HERE / "aggregate-v2.json", "aggregate campaign path")
    result = audit_header(summary_path)
    summary = read(summary_path)
    historical = historical_context()
    ...
    for entry in summary["reports"]:
        ...
        audit_receipt_path = folder / "audit-v2-execution-receipt.json"
        audit_receipt = read(audit_receipt_path)
        audit_stdout = bind(folder / "audit-v2-stdout.txt", audit_receipt["stdout_sha256"])
...
def main():
    ...
    output = path.parent / "independent-audit-v2.json"
