...
def _fresh_audit(folder, seed, summary_sha, helper, inputs, auditor_sha, test_receipt_sha):
    ...
    receipt_path = folder / "audit-execution-receipt.json"
    receipt = helper._read(helper._bind(receipt_path, helper._sha(receipt_path), inputs))
    stdout = helper._bind(folder / "audit-stdout.txt", receipt["stdout_sha256"], inputs)
            ...
            entries.append(
                {
                    ...
                    "audit_sha256": helper._sha(folder / "independent-audit.json"),
                    "audit_path": str(folder / "independent-audit.json"),
                    "audit_execution_receipt_sha256": helper._sha(
                        folder / "audit-execution-receipt.json"
                    ),
                    "audit_execution_receipt": str(folder / "audit-execution-receipt.json"),
...
def main():
    ...
    parser.add_argument(
        "--out",
        type=Path,
        default=Path("runs/learning/bilinear-budget8000-replication-v1/aggregate.json"),
    )
    ...
    auditor_path = helper._bind(
        out.parent / "independent-audit.py", args.auditor_sha256, context["inputs"]
    )
    ...
    for path, pinned, expected in (
        ...
        if expected == digest:
            ...
            _require(
                set(value["test_dependencies"]) == {str(p.resolve()) for p in required},
