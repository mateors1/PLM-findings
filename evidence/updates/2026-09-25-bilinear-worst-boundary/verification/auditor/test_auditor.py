...
PATH = Path(__file__).resolve().parents[1] / "bilinear-worst-boundary-v1/independent-audit.py"
SPEC = importlib.util.spec_from_file_location("worst_boundary_auditor", PATH)
...
def test_recipe_changes_only_declared_loss_gate_fields():
    ...
    new = {k: v for k, v in old.items() if k != "initial_training_loss"} | A.recipe_changes()
...
def test_new_initial_objective_is_not_forced_to_ordinary_bce():
    records = trace()
    for record in records:
        record["pre_update_loss"] = 4.0
    A.loss_contract(records, 4.0, 0.2)
    with pytest.raises(ValueError):
        A.loss_contract(records, A.INITIAL_LOSS, 0.2)


def test_diagnostic_can_increase_without_affecting_acceptance():
    A.bce_diagnostic_contract(
        {
            "initial": A.INITIAL_LOSS,
            "final": 0.9,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("used_for_updates", True),
    ],
)
def test_bce_diagnostic_endpoint_and_role_corruption(key, value):
    diagnostic = {
        "initial": A.INITIAL_LOSS,
        "final": 0.1,
        "initial_replay_exact": True,
    ...
    with pytest.raises(ValueError):
        A.bce_diagnostic_contract(diagnostic)
