...
root = Path.cwd()
run = root / "runs/learning/bilinear-budget2000-v1"
prior_path = root / "docs/experiments/2026-09-25-bilinear-residual-model-version-inventory.json"
out = root / "docs/experiments/2026-09-25-bilinear-budget2000-model-version-inventory.json"
require(not out.exists(), "immutable inventory")
require(
    sha(prior_path) == "d616cf34571847f42797124dc08fbe40dafd397777544111e29aaa8bd7d38745",
...
require(prior["registered_final_count"] == 30, "prior inventory count")
require(
...
relative = "runs/learning/bilinear-budget2000-v1/checkpoint-final.pt"
expected = dict(prior["verified_final_checkpoint_bytes"])
require(len(expected) == 30 and relative not in expected, "new distinct derivative")
...
result = {
    "schema_version": 1,
    "inventory_kind": "additive-bilinear-budget2000-derivative-and-final-byte-verification-v1",
    "observed_at_utc": datetime.now(UTC).isoformat(),
    "prior_inventory_path": prior_path.relative_to(root).as_posix(),
    "prior_inventory_sha256": sha(prior_path),
    "prior_registered_count": 30,
    "added_count": 1,
    "registered_final_count": 31,
    "verified_final_checkpoint_bytes": expected,
    "new_artifact": {
        "run_id": "bilinear-budget2000-v1",
    ...
    "limitations": [
        "Fresh byte hashes cover all31 final files; older payloads/configs not revalidated.",
...
print(json.dumps({"verified_final_count": 31, "inventory_sha256": sha(out)}))
