...
Clarified before execution: reproduce the existing SAME subject postprocessing.
...
contract: a generated subject ID remains permitted in the raw protocol path.
For terminated, protocol-valid sources, the existing SAME postprocessor removes
the subject when constructing the candidate set. Reproduce that explicit rule
and retain the full raw path; invalid/incomplete sources retain raw membership
as the old contract specifies. Teacher sets exclude the subject. Record per-source
subject removal flags so this transformation is visible; perform no other
entity deletion or repair. Uniqueness is checked on raw generated products.
