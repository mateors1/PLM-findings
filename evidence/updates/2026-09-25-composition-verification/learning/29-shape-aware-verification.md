...
**Status:** the revised integration contract is complete, independently audited
and accepted. It covers 666 HTTP responses (222 reused, 444 fresh), 666 fresh
serial references and 24 repeats. All same-shape score checks and cross-shape
path/set/selection checks pass with zero tolerance. The original campaign
remains failed under its original contract.
...
The saved first-checkpoint HTTP responses were rechecked against the new serial
reference. The remaining two checkpoints completed real new HTTP requests.
Filtering, zero limits, hydration, invalid requests, admission rejection,
disabled behavior and short-bound failures pass in all three suites. The
independent auditor reconstructed 2,664 normal HTTP source paths and checked
6,660 normal HTTP slot scores, selections and the fresh/reused accounting.

The [portable result](../experiments/2026-09-25-pair-composition-shape-aware.json)
binds the verifier, reference seal, prelaunch receipts, raw reports, independent
audit and separate acceptance. Exact sets are 191, 192 and 186 by seed, totaling
569/666 with 96.69% macro F1, matching the offline composition result.

There are two limits to the audit evidence. It authenticates saved neural
vectors rather than independently regenerating them. It also verifies nine
owned-server shutdown receipts rather than making independent socket probes;
the archived verifier establishes phase ordering, not filesystem timestamps.
These limits remain explicit in acceptance. The policy stays opt-in.
