...
SCRIPT = ROOT / "runs/learning/weak-margin-screen-v1/independent-audit.py"
spec = importlib.util.spec_from_file_location("weak_margin_independent_audit", SCRIPT)
...
def test_weak_recipe_differs_only_by_fixed_coefficient_and_run_label():
    _, _, original, _, _ = synthetic_report()
    control = audit.effective_recipe(original, "control")
    treatment = audit.effective_recipe(original, "treatment")
    assert control["run_name"] == "national_dex_rank_margin_weak_control_s1729_v1"
    assert treatment["run_name"] == "national_dex_rank_margin_m1_w0001_s1729_v1"
    assert control["model"]["symmetric_margin_loss_weight"] == 0.0
    assert treatment["model"]["symmetric_margin_loss_weight"] == 0.001
    assert control["model"]["symmetric_margin"] == treatment["model"]["symmetric_margin"] == 1.0
    treatment["run_name"] = control["run_name"]
    treatment["model"]["symmetric_margin_loss_weight"] = 0.0
    assert treatment == control


@pytest.mark.parametrize("coefficient", [0.0, 0.1, 0.01, 0.0001, 1.0])
def test_wrong_actual_treatment_coefficient_rejected_even_with_updated_hash(coefficient):
    report, training, original, tokens, validation = synthetic_report()
    report["arm"] = "treatment"
    report["config"] = audit.evaluation_recipe(audit.effective_recipe(original, "treatment"))
    report["config"]["model"]["symmetric_margin_loss_weight"] = coefficient
    report["config_hash"] = audit.canonical(report["config"])
    with pytest.raises(ValueError, match="evaluation recipe"):
        audit.evaluation_evidence(report, "treatment", training, original, tokens, validation)


def test_correct_fixed_treatment_coefficient_passes_synthetic_report():
    report, training, original, tokens, validation = synthetic_report()
    report["arm"] = "treatment"
    report["config"] = audit.evaluation_recipe(audit.effective_recipe(original, "treatment"))
    report["config_hash"] = audit.canonical(report["config"])
...
def test_historical_run_name_rejected_even_though_config_hash_omits_run_name():
    report, training, original, tokens, validation = synthetic_report()
    digest = report["config_hash"]
    report["config"]["run_name"] = "national_dex_rank_margin_control_s1729_v1"
    assert audit.canonical(report["config"]) == digest
    with pytest.raises(ValueError, match="evaluation recipe"):
        audit.evaluation_evidence(report, "control", training, original, tokens, validation)


def test_old_raw_config_is_authenticated_before_default_migration():
    _, _, original, _, _ = synthetic_report()
    original["config"]["model"]["symmetric_margin_loss_weight"] = 0.0
    original["config"]["model"]["symmetric_margin"] = 1.0
    with pytest.raises(ValueError, match="original raw config identity"):
        audit.effective_recipe(original, "control")
