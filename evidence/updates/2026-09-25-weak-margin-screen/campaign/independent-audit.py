"""Independent stdlib audit of the declared fresh-control weak-margin screen.
...
ROOT = HERE.parents[2]
INPUTS = {}
PLAN_SHA = "3c0449a3154492ab4c053674b6cf76ad3a00e18974e07f05bfbb638ada0973d2"
...
RUNNER_SHA = "df4b41f30d4a399cda88e62cc8334ad58522da7de191741ced31786caef8dff5"
RUNNER_TEST_SHA = "17ffaefec9344049189df18de8ca08c9c4130e27286cef5b8062ab288278ae54"
HELPER_SHA = "a5d50dc4b57277bde1cefb1cf533b4958b28b04eb515182f855b3a4aa81311b8"
FAILED_SCREEN_SHA = "39aafceac5c81d73bc7f38b2cd4f619c23d1480829456e031a352c5c1c6bd701"
GRADIENT_PORTABLE_SHA = "d44e1c656a09f53f1b32f1495b9a39ebc35a03085b3fb0639dc5bdaa0b4c87ad"
GRADIENT_ACCEPTANCE_SHA = "a66863691b2573e5fa8e1a44549b561ab2b46aaece9033e2dba64e8161700aed"
...
def effective_recipe(original, arm):
    ...
    expected["model"].update(
        symmetric_margin_loss_weight=0.0 if arm == "control" else 0.001, symmetric_margin=1.0
    )
...
def prior_campaign_evidence(summary, path):
    """Bind the motive and old failure without substituting either old comparator."""
    pins = {
        "helper_sha256": HELPER_SHA,
        "failed_screen_summary_sha256": FAILED_SCREEN_SHA,
        "gradient_portable_sha256": GRADIENT_PORTABLE_SHA,
        "gradient_acceptance_sha256": GRADIENT_ACCEPTANCE_SHA,
    }
    for key, digest in pins.items():
        check(summary[key] == digest, "declared prior input pin: " + key)
    old_directory = ROOT / "runs/learning/symmetric-margin-screen-v1"
    bind(old_directory / "summary.script.py", HELPER_SHA)
    bind(path.with_suffix(".helper.py"), HELPER_SHA)
    failed = read(old_directory / "summary.json", FAILED_SCREEN_SHA)
    bind(path.with_suffix(".failed-screen.json"), FAILED_SCREEN_SHA)
    check(
        failed["complete"] is True and failed["gate"]["numerical_gates_passed"] is False,
        "prior failure remains failure",
    )
    portable = read(
        ROOT / "docs/experiments/2026-09-25-margin-gradient-diagnosis.json", GRADIENT_PORTABLE_SHA
    )
    bind(path.with_suffix(".gradient-portable.json"), GRADIENT_PORTABLE_SHA)
    gradient_directory = ROOT / "runs/learning/margin-gradient-diagnosis-v1"
    acceptance = read(gradient_directory / "acceptance.json", GRADIENT_ACCEPTANCE_SHA)
    bind(path.with_suffix(".gradient-acceptance.json"), GRADIENT_ACCEPTANCE_SHA)
    check(
        portable["acceptance_sha256"] == GRADIENT_ACCEPTANCE_SHA
        and portable["diagnostic_accepted"] is True
        and acceptance["diagnostic_accepted"] is True
        and acceptance["model_promoted"] is False
        and acceptance["protected_test_measured"] is False,
        "gradient diagnostic acceptance scope",
    )
    diagnostic = read(gradient_directory / "summary.json", acceptance["summary_sha256"])
    audit = read(
        gradient_directory / "independent-audit.json", acceptance["independent_audit_sha256"]
    )
    check(
        diagnostic["complete"] is True
        and audit["audit_passed"] is True
        and audit["summary_sha256"] == acceptance["summary_sha256"]
        and diagnostic["source_archive_sha256"] == SOURCE_SHA,
        "prior diagnostic saved-evidence chain",
    )
    bind(gradient_directory / "independent-audit.py", audit["script_sha256"])
    bind(gradient_directory / "summary.script.py", diagnostic["script_sha256"])
    bind(gradient_directory / "record-acceptance.py", acceptance["acceptance_script_sha256"])


def auditor_test_evidence(own_sha):
    directory = ROOT / "runs/learning/weak-margin-auditor-tests-v1"
    ...
    check(receipt["tested_auditor_sha256"] == own_sha, "auditor differs from tested bytes")
    bind(directory / "test_auditor.py", receipt["tests_sha256"])
    bind(directory / "stdout.txt", receipt["stdout_sha256"])
    check(type(receipt["passed_count"]) is int and receipt["passed_count"] > 0, "synthetic count")
    check(
...
def runner_test_evidence():
    directory = ROOT / "runs/learning/weak-margin-runner-tests-v1"
    receipt = read(directory / "test-receipt.json", RUNNER_TEST_SHA)
    check(
        receipt["passed"] is True
        and receipt["experiment_evaluation_executed"] is False
        ...
        "focused runner tests/preflight",
    )
    for filename, digest in receipt["tested_files_sha256"].items():
        bind(ROOT / filename, digest)
    bind(directory / "stdout.txt", receipt["stdout_sha256"])
...
def main():
    ...
    check(
        summary["campaign_version"] == "plm-weak-margin-stage1-v1"
    ...
    snapshot_names = {
        ...
        "helper.py",
        "failed-screen.json",
        "gradient-portable.json",
        "gradient-acceptance.json",
    }
    ...
    bind(ROOT / "scripts/run_weak_margin_screen.py", summary["snapshot_sha256"]["script.py"])
    bind(ROOT / "docs/experiments/2026-09-25-weak-margin-plan.md", PLAN_SHA)
    ...
    prior_campaign_evidence(summary, path)
    test_receipt_sha = auditor_test_evidence(own_sha)
    ...
    result = {
        ...
        "plan_sha256": PLAN_SHA,
        "auditor_test_receipt_sha256": test_receipt_sha,
